...
public class RefreshableView extends LinearLayout {
    ...
    private boolean isScrollBottom = false;// 活动到底部
    ...
    private void init() {
        scroller = new Scroller(mContext);//滑动对象，
        refreshView = LayoutInflater.from(mContext).inflate(R.layout.refresh_top_item, null);//刷新视图顶端的的view
        refreshIndicatorView = (ImageView) refreshView.findViewById(R.id.indicator);//指示器view
        bar = (ProgressBar) refreshView.findViewById(R.id.progress);//刷新bar
        downTextView = (TextView) refreshView.findViewById(R.id.refresh_hint);//下拉显示text
        timeTextView = (TextView) refreshView.findViewById(R.id.refresh_time);//下来显示时间
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //记录下此刻y坐标
                this.lastY = y;
                if (m > 6 && canScrollDown()) {
                    return true;
                } else if (m < -6 && isScrollBottom) {
                    return true;
                }
                break;
            case MotionEvent.ACTION_UP:
    ...
    private boolean canScrollUp() {
        ...
        if (getChildCount() > 1) {
            childView = this.getChildAt(1);
            if (childView instanceof ListView) {
                Log.i(TAG, "getScrollY() = " + childView.getScrollY());
                Log.i(TAG, "getHeight() = " + childView.getHeight());
                int top = ((ListView) childView).getChildAt(0).getTop();
                int pad = ((ListView) childView).getListPaddingTop();
                if ((Math.abs(top - pad)) < 3 && ((ListView) childView).getFirstVisiblePosition() == 0) {
                    return true;
                ...
                Log.i(TAG, "getScrollY() = " + childView.getScrollY());
                Log.i(TAG, "getHeight() = " + childView.getHeight());
                Log.i(TAG, "getY() = " + childView.getY());
                Log.i(TAG, "getScaleY() = " + childView.getScaleY());
                Log.i(TAG, "getBottom() = " + childView.getBottom());
                if (((ScrollView) childView).getScrollY() == childView.getHeight()) {
                    return true;
                } else {
                    return false;
                }
